              ...
              <div className="space-y-3">
                ...
                {planId && (
                  <div className="flex items-center gap-3">
                    ...
                    {['sending', 'analyzing', 'parsing'].includes(step) && <div className="h-5 w-5 rounded-full border-2 border-gray-600"></div>}
                    {step === 'done' && <div className="h-5 w-5 rounded-full bg-green-500 flex items-center justify-center"><svg className="w-3 h-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg></div>}
                    <span className={step === 'saving' ? 'text-blue-400' : ['sending', 'analyzing', 'parsing'].includes(step) ? 'text-gray-500' : 'text-green-400'}>Saving tasks to database</span>
                  </div>
                )}
              </div>
